...
else if (isset($_GET['edit_forum']))
{
	...
	if (isset($_POST['save']))
	{
		...
		{
			$query = array(
				'SELECT'	=> 'g.g_id, g.g_read_board, g.g_post_replies, g.g_post_topics, g.g_read_topics_titles',
				'FROM'		=> 'groups AS g',
			...
			while ($cur_group = $forum_db->fetch_assoc($result))
			{
				...
				$perms_default = array(
					...
					'read_topics_titles'=>	$cur_group['g_read_topics_titles']
				);
				...
				$perms_old = array(
					...
					'read_topics_titles'=>	$_POST['read_topics_titles_old'][$cur_group['g_id']]
				);
				...
				$perms_new = array(
					...
					'read_topics_titles'=>	isset($_POST['read_topics_titles_new'][$cur_group['g_id']]) ? '1' : '0'
				);
...
<?php
	...
	$query = array(
		'SELECT'	=> 'g.g_id, g.g_title, g.g_read_board, g.g_post_replies, g.g_post_topics, g.g_read_topics_titles, fp.read_forum, fp.post_replies, fp.post_topics, fp.read_topics_titles',
		'FROM'		=> 'groups AS g',
		'JOINS'		=> array(
			array(
	...
	while ($cur_perm = $forum_db->fetch_assoc($result))
	{
		...
		$read_topics_titles = (($cur_perm['g_read_topics_titles'] == '0' && $cur_perm['read_topics_titles'] == '1') || ($cur_perm['g_read_topics_titles'] == '1' && $cur_perm['read_topics_titles'] != '0')) ? true : false;
		...
		$read_topics_titles_def = (($read_topics_titles && $cur_perm['g_read_topics_titles'] == '0') || (!$read_topics_titles && ($cur_perm['g_read_topics_titles'] == '' || $cur_perm['g_read_topics_titles'] == '1'))) ? false : true;
...
<?php ($hook = get_hook('afo_edit_forum_pre_cur_group_read_topics_titles_permission')) ? eval($hook) : null; ?>
						<div class="mf-item">
							<input type="hidden" name="read_topics_titles_old[<?php echo $cur_perm['g_id'] ?>]" value="<?php echo ($read_topics_titles) ? '1' : '0' ?>" />
							<span class="fld-input"><input type="checkbox" id="fld<?php echo ++$forum_page['fld_count'] ?>" name="read_topics_titles_new[<?php echo $cur_perm['g_id'] ?>]" value="1"<?php if ($read_topics_titles) echo ' checked="checked"'; echo ($cur_forum['redirect_url'] != '') ? ' disabled="disabled"' : ''; ?> /></span>
							<label for="fld<?php echo $forum_page['fld_count'] ?>"<?php if (!$read_topics_titles_def) echo ' class="warn"'; ?>><?php echo $lang_admin_forums['Read topics titles'] ?> <?php if (!$read_topics_titles_def) echo $lang_admin_forums['Not default'] ?></label>
						</div>
