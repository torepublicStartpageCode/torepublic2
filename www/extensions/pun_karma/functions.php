...
{
	global $forum_db, $forum_user, $lang_pun_karma, $forum_config;
	...
	if (!$forum_db->num_rows($result) and $forum_user['karma']>=$forum_config['o_nya_hide_treshold'])
	{
		$query = array(
		...
		if($forum_user['karma']>=$forum_config['o_nya_hide_treshold'])
		{
			message($lang_pun_karma['Already voted']);
		}else
		{
			message($lang_pun_karma['Not enough karma']);
		}
}
...
function karma_minus($post_id)
{
	...
	if (!$forum_db->num_rows($result) and $forum_user['karma']>=$forum_config['o_nya_hide_treshold'])
	{
		$minus_karma_vote= -1*$forum_config['o_nya_minus_karma_weight'];
			$query = array(
			'INSERT'		=> 'user_id, post_id, mark, updated_at',
			'INTO'			=> 'pun_karma',
			'VALUES'		=> $forum_user['id'].', '.$post_id.', -1, '.time()
		...
		$query = array(
			'UPDATE'		=> 'posts',
			'SET'			=> 'karma = karma + '.$minus_karma_vote,
			'WHERE'			=> 'id = '.$post_id
		);
		$forum_db->query_build($query) or error(__FILE__, __LINE__);
...
function karma_cancel($post_id)
{
	...
	if ($prev_mark<0)
	{
		$prev_mark=-1*$forum_config['o_nya_minus_karma_weight'];
	}
	$query = array(
		'UPDATE'		=> 'posts',
