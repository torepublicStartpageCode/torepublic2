...
if (!$_SESSION['NOT_BOT'] )
	{
		...
		{
			$_SESSION['first_number'] = rand(0,100);
			$_SESSION['second_number']= rand(0,100);
			echo "<!DOCTYPE html>";
			echo "<html>";
			echo "<head><meta charset=\"utf-8\"><title>ToRepublic Bot Check</title></head>";
			echo "<body>";
			echo "<h1>Anti-DDOS Test</h1>";
			echo "<h3> ".$_SESSION['first_number']." plus ".$_SESSION['second_number']."  = </h3>";
		?>
			<form method="POST">
		...
		<?php
			...
			$proper_result = $_SESSION['first_number'] + $_SESSION['second_number'];
			if ($_POST['result']==$proper_result){
				$_SESSION['NOT_BOT']=1;	
				$url='./index.php';
				header('Location: ' . $url);
				die();
