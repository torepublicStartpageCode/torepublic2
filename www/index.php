...
function get_forum_availability($forum_read_perm, $forum_write_perm)
{
	global $forum_user, $lang_index;
	$ret_str = '';
	if ($forum_read_perm==1 || $forum_read_perm==NULL)
		$ret_str=$ret_str.'<font color="green"> '.$lang_index['Can read'].'</font>';
	if ($forum_write_perm==1 || ($forum_write_perm==NULL && !$forum_user['is_guest']) )
		$ret_str=$ret_str.'<font color="green"> '.$lang_index['Can write'].'</font>';
	return $ret_str;
}
...
$query = array(
	'SELECT'	=> 'c.id AS cid, c.cat_name, f.id AS fid, f.forum_name, f.forum_desc, f.redirect_url, f.moderators, f.num_topics, f.num_posts, f.last_post, f.last_post_id, f.last_poster, fp.read_forum, fp.post_replies',
	...
	'WHERE'		=> '(fp.read_topics_titles IS NULL OR fp.read_topics_titles=1) AND NOT (c.id ='.$forum_config['o_trade_category_id'].' AND f.num_topics<1)',
...
<?php
	...
	else
	{
		...
		if ($cur_forum['forum_desc'] != '')
			$forum_page['item_subject']['desc'] = $cur_forum['forum_desc'].get_forum_availability($cur_forum['read_forum'], $cur_forum['post_replies']);
...
if ($forum_config['o_users_online'] == '1' or $forum_user['is_admmod'])
{
	// Fetch users online info and generate strings for output
	$query = array(
		'SELECT'	=> 'o.user_id, o.ident',
		'FROM'		=> 'online AS o',
		'WHERE'		=> 'o.idle=0 AND o.user_id!=2 AND o.user_id!=281',  //zmodyfikowane by nie bylo widac Mbanka, ktory chce zaaresztowac wszystkich, ale boi sie zemsty podziemia + luc
		'ORDER BY'	=> 'o.ident'
	);
