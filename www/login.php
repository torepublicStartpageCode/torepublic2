...
// Login
if (isset($_POST['form_sent']) && empty($action) || $_SESSION['GPG_VERIFICATION_REQUIRED'] && isset($_SESSION['form_username']) && isset($_SESSION['form_password'] ))
{
		if (isset($_SESSION['form_username']) && isset($_SESSION['form_password'] ))
		{ 
			$form_username = $_SESSION['form_username'];
			$form_password = $_SESSION['form_password'];
		}
		else
		{
		...
		$query = array(
			'SELECT'	=> 'u.id, u.group_id, u.password, u.salt, u.pubkey',
			'FROM'		=> 'users AS u'
		);
		...
		list($user_id, $group_id, $db_password_hash, $salt, $pubkey) = $forum_db->fetch_row($result);
	...
	if (empty($errors))
	{
		...
		if (!isset($_POST['req_CLEARTEXT'])){
			...
			if (!$_SESSION['GPG_CIPHERTEXT']){$_SESSION['GPG_CIPHERTEXT']='SORRY, YOUR PUBKEY IS FAULTY';}
		}
...
?>
	<div class="main-head">
		<h2 class="hn"><span><?php $lang_login['New password request'] ?></span></h2>
	</div>
...
<?php if (!$_SESSION['GPG_VERIFICATION_REQUIRED']) {?>
			<div class="frm-group group<?php echo ++$forum_page['group_count'] ?>">
...
<?php  } ?>			
			...
			<?php if ($_SESSION['GPG_VERIFICATION_REQUIRED']) {?>
				...
				<div class="sf-set set<?php echo ++$forum_page['item_count']; if ($forum_config['o_regs_verify'] == '0') echo ' prepend-top'; ?>" style="height:40pt">
					<div class="sf-box text required" >
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo 'Ciphertext' ?></span> </label><br />
						<span class="fld-input" ><textarea cols="60" rows="3" id="fld<?php echo $forum_page['fld_count'] ?>" name="ciphertext"><?php echo $_SESSION['GPG_CIPHERTEXT']; ?></textarea></span>
					</div>
				</div>
				<br/><br/><br/>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text required">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo 'Cleartext'; ?></span></label><br />
						<span class="fld-input"><input type="text" id="fld<?php echo $forum_page['fld_count'] ?>" name="req_CLEARTEXT" size="10" required /></span>
					</div>
				</div>
			<?php  } ?>
			<div class="frm-buttons">
