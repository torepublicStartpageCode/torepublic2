...
if (!$forum_user['is_guest'] && 
	$forum_user['karma'] < $forum_config['karma_post_stop'] && $fid != $forum_config['o_guest_forum'])
{
...
else
{
	...
	//ADMINISTRATIVE FEATURE - UPDATE BTC PRICE
	...
	//ADMINISTRATIVE FEATURE - TRANSFERS NEW MEMBERS TO MEMBERS
	
	// NEw members to members
	$queryMove = array(
		'UPDATE' 	=>  'users',
		'SET'		=>	'group_id=3',
		'WHERE'		=>	'group_id='.$forum_config['o_default_user_group'].' AND karma>'.$forum_config['o_nya_new_member_treshold']
		);
	$forum_db->query_build($queryMove) or error(__FILE__, __LINE__);	

	// Members to New Members
	$queryMove = array(
		'UPDATE' 	=>  'users',
		'SET'		=>	'group_id='.$forum_config['o_default_user_group'],
		'WHERE'		=>	'group_id=3 AND karma<'.$forum_config['o_nya_new_member_treshold']
		);
	$forum_db->query_build($queryMove) or error(__FILE__, __LINE__);	
}
...
if (isset($_POST['form_sent']))
{
	...
	{
		$username = "_".forum_trim($_POST['req_username']);
	...
	else if (isset($_POST['allow_guests']))// and $_POST['allow_guests']==1)
		{
			echo 'allow_guests';
			$visibility =1;
		}
	...
	else if  (isset($_POST['allow_new_members']))// and $_POST['allow_new_members']==1)
		{
			echo 'allow_new_members';
			$visibility =2;
		}
		else if  (!isset($_POST['allow_new_members']))// and !$_POST['allow_new_members']==1)
		{
	...
	else if  (isset($_POST['allow_members']))// and $_POST['allow_members']==1 )
		{
			$visibility=3;
		}
	else if (!isset($_POST['allow_members']))// and !$_POST['allow_members']==1 )
	...
	if (empty($errors) && !isset($_POST['preview']))
	{
		...
		echo "TEST VISIBILITY  ".$visibility;
		//echo isset($_POST['allow_guests']) , isset($_POST['allow_new_members']), isset($_POST['allow_members']);
...
if (!$forum_user['is_guest'])// && $forum_config['o_subscriptions'] == '1')
{
	...
	// if group_id > N , visibility for N checkbox
	if ($fid>0)
	{
		//sciagnij informacje o widocznosci forum dla gosci
			$query = array(
				'SELECT'	=> 'fp.read_forum',
				'FROM'		=> 'forum_perms AS fp',
				'WHERE'		=> 'fp.forum_id='.$fid.' AND fp.group_id=2'
				);

			($hook = get_hook('vt_qr_get_post_info')) ? eval($hook) : null;
			$result = $forum_db->query_build($query) or error(__FILE__, __LINE__);
			$forum_perms_guest_info = $forum_db->fetch_assoc($result);
			
			if (!isset($forum_perms_guest_info['read_forum']))
				{ $guest_perms =1;}
			
		//sciagnij informacje o widocznosci forum dla new member
			$query = array(
				'SELECT'	=> 'fp.read_forum',
				'FROM'		=> 'forum_perms AS fp',
				'WHERE'		=> 'fp.forum_id='.$fid.' AND fp.group_id=7'
				);

			($hook = get_hook('vt_qr_get_post_info')) ? eval($hook) : null;
			$result = $forum_db->query_build($query) or error(__FILE__, __LINE__);
			$forum_perms_new_member_info = $forum_db->fetch_assoc($result);
			
			if (!isset($forum_perms_new_member_info['read_forum']))
				{ $new_member_perms =1;}
				
			$members_perms =1;
			
			
		if ($forum_user['group_id']==7)  // new member group id
		{
			
			$forum_page['checkboxes']['guest_visibility'] = '<div class="mf-item"><span class="fld-input"><input type="checkbox" id="fld'.(++$forum_page['fld_count']).'" name="allow_guests" value="1"'.($guest_perms? ' checked="checked"' : '').' /></span> <label for="fld'.$forum_page['fld_count'].'">'.($is_subscribed ? $lang_post['Allow Guests'] : $lang_post['Guests visibility']).'</label></div>';
			$forum_page['checkboxes']['new_member_visibility'] = '<div class="mf-item"><span class="fld-input"><input TYPE=CHECKBOX DISABLED id="fld'.(++$forum_page['fld_count']).'" name="allow_new_members" value="1"'.($new_member_perms ? ' checked="checked"' : '').' /></span> <label for="fld'.$forum_page['fld_count'].'">'.($is_subscribed ? $lang_post['Allow New Members'] : $lang_post['New Members visibility']).'</label></div>';
			$forum_page['checkboxes']['member_visibility'] = '<div class="mf-item"><span class="fld-input"><input TYPE=CHECKBOX DISABLED id="fld'.(++$forum_page['fld_count']).'" name="allow_members" value="1"'.($members_perms ? ' checked="checked"' : '').' /></span> <label for="fld'.$forum_page['fld_count'].'">'.($is_subscribed ? $lang_post['Allow Members'] : $lang_post['Members visibility']).'</label></div>';
			$_POST['allow_members'] =1;
			$_POST['allow_new_members']=1;
		}
		else if ($forum_user['group_id']==3) // member group id
		{
			//sciagnij informacje o widocznosci forum dla gosci i New Members
			$forum_page['checkboxes']['guest_visibility'] = '<div class="mf-item"><span class="fld-input"><input type="checkbox" id="fld'.(++$forum_page['fld_count']).'" name="allow_guests" value="1"'.($guest_perms ? ' checked="checked"' : '').' /></span> <label for="fld'.$forum_page['fld_count'].'">'.($is_subscribed ? $lang_post['Allow Guests'] : $lang_post['Guests visibility']).'</label></div>';
			$forum_page['checkboxes']['new_member_visibility'] = '<div class="mf-item"><span class="fld-input"><input type="checkbox" id="fld'.(++$forum_page['fld_count']).'" name="allow_new_members" value="1"'.($new_member_perms ? ' checked="checked"' : '').' /></span> <label for="fld'.$forum_page['fld_count'].'">'.($is_subscribed ? $lang_post['Allow New Members'] : $lang_post['New Members visibility']).'</label></div>';
			$forum_page['checkboxes']['member_visibility'] = '<div class="mf-item"><span class="fld-input"><input TYPE=CHECKBOX DISABLED id="fld'.(++$forum_page['fld_count']).'" name="allow_members" value="1"'.($members_perms ? ' checked="checked"' : '').' /></span> <label for="fld'.$forum_page['fld_count'].'">'.($is_subscribed ? $lang_post['Allow Members'] : $lang_post['Members visibility']).'</label></div>';
			$_POST['allow_members']=1;
		}
		else if ($forum_user['is_admmod'] or $forum_user['group_id']==8) // old member group id
		{
			//sciagnij informacje o widocznosci forum dla gosci , New Members i Members
			$forum_page['checkboxes']['guest_visibility'] = '<div class="mf-item"><span class="fld-input"><input type="checkbox" id="fld'.(++$forum_page['fld_count']).'" name="allow_guests" value="1"'.($guest_perms ? ' checked="checked"' : '').' /></span> <label for="fld'.$forum_page['fld_count'].'">'.($is_subscribed ? $lang_post['Allow Guests'] : $lang_post['Guests visibility']).'</label></div>';
			$forum_page['checkboxes']['new_member_visibility'] = '<div class="mf-item"><span class="fld-input"><input type="checkbox" id="fld'.(++$forum_page['fld_count']).'" name="allow_new_members" value="1"'.($new_member_perms ? ' checked="checked"' : '').' /></span> <label for="fld'.$forum_page['fld_count'].'">'.($is_subscribed ? $lang_post['Allow New Members'] : $lang_post['New Members visibility']).'</label></div>';
			$forum_page['checkboxes']['member_visibility'] = '<div class="mf-item"><span class="fld-input"><input type="checkbox" id="fld'.(++$forum_page['fld_count']).'" name="allow_members" value="1"'.($members_perms ? ' checked="checked"' : '').' /></span> <label for="fld'.$forum_page['fld_count'].'">'.($is_subscribed ? $lang_post['Allow Members'] : $lang_post['Members visibility']).'</label></div>';
		}
	}
}
