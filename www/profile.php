...
else if (isset($_POST['form_sent']))
{
	...
	switch ($section)
	{
		case 'identity':
		{
			$form = extract_elements(array('realname', 'url', 'location', 'jabber', 'icq', 'msn', 'aim', 'yahoo', 'facebook', 'twitter', 'linkedin', 'skype' , 'btcaddress' , 'pubkey' , 'invited' , 'invitedBy'));
	...
	($forum_user['g_moderator'] != '1' || $forum_user['g_mod_edit_users'] == '0' || $user['g_id'] == FORUM_ADMIN || $user['g_moderator'] == '1'))
{
	...
	if ($user['pubkey']!='')
		$forum_page['user_contact']['forum-pubkey'] = '<li><span>RSA public key: <textarea cols="70" rows ="10" readonly wrap="soft" syle="font-size: 6pt; font-family: Courier">'.htmlspecialchars(validate_pubkey($user['pubkey'])).'</textarea></span></li>';
	if ($user['btcaddress']!='')
		$forum_page['user_contact']['forum-btcaddress'] = '<li><span>BTC address: '.htmlspecialchars($user['btcaddress']).'</span></li>';
	if ($user['invitedBy']!='')
		$forum_page['user_contact']['forum-invitedBy'] = '<li><span>Invited by: '.$user['invitedBy'].'</span></li>';
	if ($user['invited']!=NULL)
		$forum_page['user_contact']['forum-invited'] = '<li><span>Invited users: '.$user['invited'].'</span></li>';
	//if ( !$forum_user['is_guest'])
	//	$forum_page['user_contact']['escrow'] = '<li><span>Escrow: <a href="escrows.php?action=start_new_escrow&user_id='.$user['id'].'" >'.$lang_profile['Start new escrow'].'</a></span></li>';

	if ($forum_page['own_profile'])
	{	
		$forum_page['user_contact']['forum-invitation-form'] = '<li><span><form action=profile.php?id='.$user['id'].' method="post">Username: <input type="text" name="invited_username" value="'.$_POST['invited_username'].'"><input type="submit" value="Generate"></form></span></li>';
		if (isset($_POST['invited_username']))
			{
				$invite = get_proper_invitation($user['username'],trim($_POST['invited_username']) );
				$forum_page['user_contact']['forum-invitation'] = '<li><span>Invitation code: '.$invite.'</span></li>';
			}
	}
	if (!$forum_user['is_guest'] && $forum_user['karma']>intval($forum_config['o_nya_hide_treshold']) && $forum_user['id']!=$user['id'])
	{
		
		$forum_page['user_contact']['forum-transfer-karma-form'] = '<li><span><form action=profile.php?id='.$user['id'].' method="post">Transfer karma: <input type="number" name="karma_transfer" value="'.$_POST['karma_transfer'].'" min="0" max="'.($forum_user['karma']-1).'"><input type="submit" value="Transfer"></form></span></li>';
		if (isset($_POST['karma_transfer']) && is_numeric($_POST['karma_transfer']))
		{
			transfer_karma($forum_user['id'], $user['id'], $forum_user['karma'], $user['karma'], $_POST['karma_transfer']);
			$forum_page['user_contact']['forum-karma-transfer'] = '<li><span>You transfered him: '.intval($_POST['karma_transfer']).' point</span></li>';
		}
	}
	...
	// Setup search links
	if ($forum_user['g_search'] == '1' ||  $forum_user['g_id']==8)
	{
	...
	//Setup trade reputation
	require FORUM_ROOT.'/include/reputation.php';
	$forum_page['user_reputation'] = array();
	if ($forum_user['g_search'] == '1' )
	{
		$user_reputation = get_user_reputation($user['id']);
		$forum_page['user_reputation']['reputation_result'] = '<li class="first-item">'.$user_reputation.'</li>';
		$forum_page['user_reputation']['reputation_texts'] = '<li>'.get_reputation_html_string($user['id']).'</li>';
	}
...
<?php endif; ($hook = get_hook('pf_view_details_pre_user_reputation_info')) ? eval($hook) : null; ?>
<?php if (!empty($forum_page['user_reputation'])): ?>
			<div class="ct-set data-set set<?php echo ++$forum_page['item_count'] ?>">
				<div class="ct-box data-box">
					<h3 class="ct-legend hn"><span><?php echo $lang_profile['Reputation'] ?></span></h3>
					<ul class="data-box">
						<?php echo implode("\n\t\t\t\t\t\t", $forum_page['user_reputation']) ?>
					</ul>
				</div>
			</div>
...
else
{
	...
	if ($section == 'about')
	{
		...
		if ($user['pubkey']!='')
			$forum_page['user_contact']['forum-pubkey'] = '<li><span>GPG public key: <textarea cols="70" rows ="10" readonly wrap="soft" syle="font-size: 4pt; font-family:Monospace">'.htmlspecialchars(validate_pubkey($user['pubkey'])).'</textarea></span></li>';
		if ($user['btcaddress']!='')
			$forum_page['user_contact']['forum-btcaddress'] = '<li><span>BTC address: '.htmlspecialchars($user['btcaddress']).'</span></li>';
		if ($user['invitedBy']!='')
			$forum_page['user_contact']['forum-invitedBy'] = '<li><span>Invited by: '.$user['invitedBy'].'</span></li>';
		...
		if ($forum_page['own_profile'])
			{
			if($user['karma']> $user['invited'] or $forum_user['is_admmod'])
				{
				$forum_page['user_contact']['forum-invitation-form'] = '<li><span><form action=profile.php?id='.$user['id'].' method="post">Username: <input type="text" name="invited_username" value="'.$_POST['invited_username'].'"><input type="submit" value="Generate"></form></span></li>';
				if (isset($_POST['invited_username']))
					{
		...
		*/
		...
		if ($user['skype'] !='')
			$forum_page['user_contact']['skype'] = '<li><span>'.'BitMessage'.': <strong>'.forum_htmlencode(($forum_config['o_censoring'] == '1') ? censor_words($user['skype']) : $user['skype']).'</strong></span></li>';
		if ($user['icq'] !='')
		...
		//	$forum_page['user_options']['add_trade_reputation'] = '<span'.(empty($forum_page['user_options']) ? ' class="first-item"' : '').'><a href="'.forum_link($forum_url['add_trade_reputation'], $id).'">'.(($forum_page['own_profile']) ? $lang_profile['Add trade reputation'] : sprintf($lang_profile['Add trade reputation'], forum_htmlencode($user['username']))).'</a></span>';

		
		if ($forum_page['own_profile'])	
			$forum_page['user_options']['change_pubkey'] = '<span'.(empty($forum_page['user_options']) ? ' class="first-item"' : '').'><a href="'.forum_link($forum_url['change_password'], $id).'">'.(($forum_page['own_profile']) ? $lang_profile['Change your public key'] : sprintf($lang_profile['Change user public key'], forum_htmlencode($user['username']))).'</a></span>';
...
<?php endif; ($hook = get_hook('pf_change_details_identity_karma')) ? eval($hook) : null; ?>
<?php if ($forum_user['g_id'] == FORUM_ADMIN): ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo 'Edit karma' ?></span></label><br />
						<span class="fld-input"><input type="number" id="fld<?php echo $forum_page['fld_count'] ?>" name="karma" value="<?php echo $user['karma'] ?>" size="8" maxlength="8" /></span>
					</div>
				</div>
...
<?php ($hook = get_hook('pf_change_details_identity')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_profile['Bitcoin address'] ?></span></label><br />
						<span class="fld-input"><input id="fld<?php echo $forum_page['fld_count'] ?>" type="text" name="form[btcaddress]" value="<?php echo(isset($form['btcaddress']) ? validate_btcaddress(forum_htmlencode($form['btcaddress'])) : validate_btcaddress(forum_htmlencode($user['btcaddress']))) ?>" size="35" maxlength="35" /></span>
					</div>
				</div>
...
<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
	<div class="sf-box text">
		<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo 'BitMessage'; ?></span></label><br />
...
<?php ($hook = get_hook('pf_change_details_identity')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_profile['Public key'] ?></span></label><br />
						<!--<span class="fld-input"><input id="fld<?php// echo $forum_page['fld_count'] ?>" type="text" name="form[pubkey]" value="<?php //echo(isset($form['pubkey']) ? validate_pubkey(forum_htmlencode($form['pubkey'])) : validate_pubkey(forum_htmlencode($user['pubkey']))) ?>" size="35" maxlength="3500" /></span>-->
						<span class="fld-input"><textarea id="fld<?php echo $forum_page['fld_count'] ?>" cols="60" rows="2" name="form[pubkey]" >"<?php echo(htmlspecialchars(isset($form['pubkey']) ? validate_pubkey(forum_htmlencode($form['pubkey'])) : validate_pubkey(forum_htmlencode($user['pubkey'])))) ?>"</textarea> </span>
					</div>
				</div>
