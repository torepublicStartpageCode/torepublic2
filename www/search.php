...
else if (isset($_GET['action']))
{
	...
	else if($forum_user['is_admmod'] or  $forum_user['group_id']==8 or $action == 'show_new' or $action == 'show_recent')
	{
		$value = null;
		...
		if ($action == 'show_user_posts' || $action == 'show_user_topics' || $action == 'show_subscriptions' || $action == 'show_forum_subscriptions')
		{
			...
			if ($value < 2 or (check_usergroup($_GET['user_id'])==1 and $_GET['user_id']!=$forum_user['id']))
				message($lang_common['Bad request']);
