...
############
	...
	$topic_visibility_info = $forum_db->fetch_assoc($result);
	

		
	if(!$topic_visibility_info['visibility'])
	{
	...
	{
		if($topic_visibility_info['visibility']==1) //widoczne dla gosci
		{
			$visible_for_guests = 1;
		}
		else if($topic_visibility_info['visibility']==2) //widoczne dla new members
		{
		...
		else if($topic_visibility_info['visibility']==3)  //widocznosc dla members
		{
			if ($forum_user['g_id']==7 or $forum_user['is_guest']) 
				{message($lang_common['No view']);}
		}
		else if($topic_visibility_info['visibility']==4)  //widocznosc dla members
		{
			if ($forum_user['g_id']==7 or $forum_user['g_id']==3 or $forum_user['is_guest']) 
				{message($lang_common['No view']);}
		}
	}
############
...
if ($pid)
{
	...
	///*
	// znajdz visibility topicku
	$id_of_the_topic = $topic_info['topic_id'];
	if ($id_of_the_topic=='') 
	{
		message($lang_common['Bad request']);
	}
...
if (!$cur_topic and !isset($visible_for_guests))
{
	message($lang_common['Bad request']);
}
